...
export default {
  ...
  computed: {
    ...
    computePacket() {
      if (this.startRetrivedOfflineDatalog) {
        return this.readText(this.gogoResponse);
      }
    },
    logRecord() {
      if (this.nRecords == 0) {
  ...
  methods: {
    ...
    readText: function (packet) {
      ...
      //   }
      //   // in with size
      //   if (this.gogoResponse.status == 4) {
      //     // this.sizeOfPacket = []
      //     size = this.gogoResponse.size;
